...
class GetFreeProxy(object):
    ...
    def freeProxyThirteen():
        ...
        https://www.kuaidaili.com/free/inha/1/
        """
        urls = ['https://www.kuaidaili.com/free/inha/%s/', 'https://www.kuaidaili.com/free/intr/%s/']
        for url_ in urls:
    ...
    def freeProxyFourteen():
        ...
        for i in urls:
            t = request.get(url+i[1:])
            li = re.findall(r'<tr>.*?>[.\d]+?<.*?>([\d]+?\.[.\d]+?)<.*?>([.\d]+?)<', t.text, re.S)
            for j in li:
                yield ':'.join(j)

    @staticmethod
    def freeProxyFifteen():
        """
        这个要牛逼上天了啊   http://www.proxylists.net
        """
        url = 'http://www.proxylists.net/proxylists.xml'
        request = WebRequest()
        text = request.get(url).text
        proxy = re.findall('<prx:ip>(.*?)<.*?port>(.*?)<', text)
        for x, y in proxy:
            yield x+':'+y

    @staticmethod
    def freeProxySixTeen():
        """
        http://www.xiladaili.com/gaoni/1/
        http://www.xiladaili.com/http/1/
        ...
        for url in urls:
            for i in range(100):
                url_ = url.format(i)
                request = WebRequest()
                text = request.get(url_).text
                proxy = re.findall("tr.*?td>(.*?)<", text, re.S)
                for j in proxy:
                    yield j

    @staticmethod
    def freeProxySeventeen():
        """
        http://www.89ip.cn/tqdl.html?num=9999
        """
        url = "http://www.89ip.cn/tqdl.html?num=9999"
        request = WebRequest()
        text = request.get(url).text
        proxy_list = re.findall(r">\s+([\d\.:]*?)<br>", text, re.S)
        for proxy in proxy_list:
            yield proxy
    ...
    def freeProxyEighteen():
        ...
        request = WebRequest()
        max_ = re.findall("class=\"title\".*?<a href=\".*?(\d+).*?\"", request.get(url).text, re.S)[0]
